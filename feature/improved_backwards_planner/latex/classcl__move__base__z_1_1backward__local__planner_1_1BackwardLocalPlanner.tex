...
Definition at line 860 of file backward\+\_\+local\+\_\+planner.\+cpp.
...
\begin{DoxyCode}
861         \{
862             Eigen::Vector3f new\_pos = Eigen::Vector3f::Zero();
863             new\_pos[0] = pos[0] + (vel[0] * cos(pos[2]) + vel[1] * cos(M\_PI\_2 + pos[2])) * dt;
864             new\_pos[1] = pos[1] + (vel[0] * sin(pos[2]) + vel[1] * sin(M\_PI\_2 + pos[2])) * dt;
865             new\_pos[2] = pos[2] + vel[2] * dt;
866             \textcolor{keywordflow}{return} new\_pos;
867         \}
\end{DoxyCode}
Here is the caller graph for this function\+:
\nopagebreak
...
458                                                                          << \textcolor{stringliteral}{" theta: "} << theta << 
      std::endl
459                                                                          << \textcolor{stringliteral}{" betta: "} << theta << 
      std::endl
460                                                                          << \textcolor{stringliteral}{" err\_x: "} << dx << std::endl
461                                                                          << \textcolor{stringliteral}{" err\_y:"} << dy << std::endl
462                                                                          << \textcolor{stringliteral}{" rho\_error:"} << rho\_error << 
      std::endl
463                                                                          << \textcolor{stringliteral}{" alpha\_error:"} << alpha\_error 
      << std::endl
464                                                                          << \textcolor{stringliteral}{" betta\_error:"} << betta\_error 
      << std::endl
465                                                                          << \textcolor{stringliteral}{" vetta:"} << vetta << std::endl
466                                                                          << \textcolor{stringliteral}{" gamma:"} << gamma << std::endl
467                                                                          << \textcolor{stringliteral}{" cmd\_vel.lin.x:"} << cmd\_vel.
      linear.x << std::endl
468                                                                          << \textcolor{stringliteral}{" cmd\_vel.ang.z:"} << cmd\_vel.
      angular.z);
469 
470                 \textcolor{keywordflow}{if} (\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ae4d7254afd19ae1f7bd8eac9f5d58b9a}{inGoalPureSpinningState\_})
471                 \{
472                     \textcolor{keywordtype}{bool} carrotHalfPlaneConstraintFailure = 
...
473 
474                     \textcolor{keywordflow}{if} (carrotHalfPlaneConstraintFailure)
475                     \{
476                         ROS\_ERROR(\textcolor{stringliteral}{"[BackwardLocalPlanner] CarrotHalfPlaneConstraintFailure detected.
...
480         
481 
482             \textcolor{comment}{// ---------------------- TRAJECTORY PREDICTION AND COLLISION AVOIDANCE ---------------------}
483             \textcolor{comment}{//cmd\_vel.linear.x=0;}
484             \textcolor{comment}{//cmd\_vel.angular.z = 0;}
485             tf::Stamped<tf::Pose> global\_pose = \hyperlink{namespacecl__move__base__z_1_1backward__local__planner_afd5bb2a0d144f7b89f4215fbcb1fbe69}{optionalRobotPose}(
      \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a4136268882a105d9e06e173d85d5c0dc}{costmapRos\_});
486 
487             \textcolor{keyword}{auto} *costmap2d = \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a4136268882a105d9e06e173d85d5c0dc}{costmapRos\_}->getCostmap();
488             \textcolor{keyword}{auto} yaw = tf::getYaw(global\_pose.getRotation());
489 
490             \textcolor{keyword}{auto} &pos = global\_pose.getOrigin();
491 
492             Eigen::Vector3f currentpose(pos.x(), pos.y(), yaw);
493             Eigen::Vector3f currentvel(cmd\_vel.linear.x, cmd\_vel.linear.y, cmd\_vel.angular.z);
494             std::vector<Eigen::Vector3f> trajectory;
495             this->\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a9e8921ca8ce379ca8ba321f82ca23cdc}{generateTrajectory}(currentpose, currentvel, 0.8 \textcolor{comment}{/*meters*/}, M\_PI / 8 \textcolor{comment}{/*
      rads*/}, 3.0 \textcolor{comment}{/*seconds*/}, 0.05 \textcolor{comment}{/*seconds*/}, trajectory);
496 
497             \textcolor{comment}{// check plan rejection}
498             \textcolor{keywordtype}{bool} acceptedLocalTrajectoryFreeOfObstacles = \textcolor{keyword}{true};
499 
500             \textcolor{keywordtype}{unsigned} \textcolor{keywordtype}{int} mx, my;
501 
502             \textcolor{keywordflow}{if} (this->\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a154043366660cc02ec758dda32817511}{enable\_obstacle\_checking\_})
503             \{
504                 \textcolor{keywordflow}{if} (\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad9cde5c85f782cab2ddb4030e3c3f2cf}{backwardsPlanPath\_}.size() > 0)
505                 \{
506                     \textcolor{keyword}{auto} &finalgoalpose = \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad9cde5c85f782cab2ddb4030e3c3f2cf}{backwardsPlanPath\_}.back();
507 
508                     \textcolor{keywordtype}{int} i = 0;
509                     \textcolor{comment}{// ROS\_INFO\_STREAM("lplanner goal: " << finalgoalpose.pose.position);}
510                     \textcolor{keywordflow}{for} (\textcolor{keyword}{auto} &p : trajectory)
511                     \{
512                         \textcolor{keywordtype}{float} dx = p[0] - finalgoalpose.pose.position.x;
513                         \textcolor{keywordtype}{float} dy = p[1] - finalgoalpose.pose.position.y;
514 
515                         \textcolor{keywordtype}{float} dst = sqrt(dx * dx + dy * dy);
516                         \textcolor{keywordflow}{if} (dst < \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_aa4ec2c87947a3c08f8278eff052e7c8c}{xy\_goal\_tolerance\_})
517                         \{
518                             ROS\_DEBUG(\textcolor{stringliteral}{"[BackwardLocalPlanner] trajectory simulation for collision checking:
       goal reached with no collision"});
519                             \textcolor{keywordflow}{break};
520                         \}
521 
522                         costmap2d->worldToMap(p[0], p[1], mx, my);
523                         \textcolor{keywordtype}{unsigned} \textcolor{keywordtype}{int} cost = costmap2d->getCost(mx, my);
524 
525                         \textcolor{comment}{// ROS\_DEBUG("[BackwardLocalPlanner] checking cost pt %d [%lf, %lf] cell[%d,%d] =
       %d", i, p[0], p[1], mx, my, cost);}
526                         \textcolor{comment}{// ROS\_DEBUG\_STREAM("[BackwardLocalPlanner] cost: " << cost);}
527 
528                         \textcolor{comment}{// static const unsigned char NO\_INFORMATION = 255;}
529                         \textcolor{comment}{// static const unsigned char LETHAL\_OBSTACLE = 254;}
530                         \textcolor{comment}{// static const unsigned char INSCRIBED\_INFLATED\_OBSTACLE = 253;}
531                         \textcolor{comment}{// static const unsigned char FREE\_SPACE = 0;}
532 
533                         \textcolor{keywordflow}{if} (costmap2d->getCost(mx, my) >= costmap\_2d::INSCRIBED\_INFLATED\_OBSTACLE)
534                         \{
535                             acceptedLocalTrajectoryFreeOfObstacles = \textcolor{keyword}{false};
536                             ROS\_WARN\_STREAM(\textcolor{stringliteral}{"[BackwardLocalPlanner] ABORTED LOCAL PLAN BECAUSE OBSTACLE
...
544                 \{
545                     ROS\_WARN(\textcolor{stringliteral}{"[BackwardLocalPlanner] Abort local - Backwards global plan size: %ld"}, 
...
548             \}
549 
550             \textcolor{keywordflow}{if} (acceptedLocalTrajectoryFreeOfObstacles)
551             \{
552                 \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a00849266e4bdd9379c3717e8a41c303e}{waiting\_} = \textcolor{keyword}{false};
553                 ROS\_DEBUG(\textcolor{stringliteral}{"[BackwardLocalPlanner] accepted local trajectory free of obstacle. Local planner
       continues."});
554                 \textcolor{keywordflow}{return} \textcolor{keyword}{true};
555             \}
556             \textcolor{keywordflow}{else} \textcolor{comment}{// that is not appceted because existence of obstacles}
557             \{
558                 \textcolor{comment}{// emergency stop for collision: waiting a while before sending error}
559                 cmd\_vel.linear.x = 0;
560                 cmd\_vel.angular.z = 0;
561 
562                 \textcolor{keywordflow}{if} (\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a00849266e4bdd9379c3717e8a41c303e}{waiting\_} == \textcolor{keyword}{false})
563                 \{
564                     \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a00849266e4bdd9379c3717e8a41c303e}{waiting\_} = \textcolor{keyword}{true};
565                     \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_abe08faab5b784fcf64ac6b3f936459f1}{waitingStamp\_} = ros::Time::now();
566                     ROS\_WARN(\textcolor{stringliteral}{"[BackwardLocalPlanner][Not accepted local plan] starting countdown"});
567                 \}
568                 \textcolor{keywordflow}{else}
569                 \{
570                     \textcolor{keyword}{auto} waitingduration = ros::Time::now() - \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_abe08faab5b784fcf64ac6b3f936459f1}{waitingStamp\_};
571 
572                     \textcolor{keywordflow}{if} (waitingduration > this->\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a0c15da985f7f051164adb643c03a0c90}{waitingTimeout\_})
573                     \{
574                         ROS\_WARN(\textcolor{stringliteral}{"[BackwardLocalPlanner][Abort local] timeout! duration %lf/%f"}, 
...
Definition at line 631 of file backward\+\_\+local\+\_\+planner.\+cpp.
...
\begin{DoxyCode}
632         \{
633             \textcolor{keywordtype}{double} lineardisterr, angleerr;
634             \textcolor{keywordtype}{bool} inCarrotRange = \textcolor{keyword}{false};
635 
636             \textcolor{comment}{// initial state check}
...
638 
639             \textcolor{keywordtype}{int} closestIndex = -1;
640             \textcolor{keywordtype}{double} minpointdist = std::numeric\_limits<double>::max();
641 
642             \textcolor{comment}{// lets set the carrot-goal in the corret place with this loop}
643             \textcolor{keywordflow}{while} (\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a2e8f2b78bc97f27c5fa431f3af2261ed}{currentCarrotPoseIndex\_} < 
...
644             \{
645                 \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad39ba029c760fc63ea286c74b5b9b795}{computeCurrentEuclideanAndAngularErrorsToCarrotGoal}
      (tfpose, lineardisterr, angleerr);
646 
647                 ROS\_DEBUG(\textcolor{stringliteral}{"[BackwardLocalPlanner] Finding initial carrot goal i=%d - error to carrot,
...
648 
649                 \textcolor{comment}{// current path point is inside the carrot distance range, goal carrot tries to escape!}
650                 \textcolor{keywordflow}{if} (lineardisterr < \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a0bbb80ce5bae865c4322869422803296}{carrot\_distance\_} && angleerr < 
...
651                 \{
652                     ROS\_DEBUG(\textcolor{stringliteral}{"[BackwardLocalPlanner] Finding initial carrot goal i=%d - in carrot Range"}, 
      \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a2e8f2b78bc97f27c5fa431f3af2261ed}{currentCarrotPoseIndex\_});
653                     inCarrotRange = \textcolor{keyword}{true};
654                     \textcolor{comment}{// we are inside the goal range}
655                 \}
656                 \textcolor{keywordflow}{else} \textcolor{keywordflow}{if} (inCarrotRange && (lineardisterr > \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a0bbb80ce5bae865c4322869422803296}{carrot\_distance\_} || angleerr > 
      \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a63e30befa09c4a67cf55086923b760c7}{carrot\_angular\_distance\_}))
657                 \{
658                     \textcolor{comment}{// we were inside the carrot range but not anymore, now we are just leaving. we want to
       continue forward (currentCarrotPoseIndex\_++)}
659                     \textcolor{comment}{// unless we go out of the carrot range}
660 
661                     \textcolor{comment}{// but we rollback last index increment (to go back inside the carrot goal scope) and
...
666                 \{
667                     ROS\_DEBUG(\textcolor{stringliteral}{"[BackwardLocalPlanner] Finding initial carrot goal i=%d - carrot out of
...
668                 \}
669 
670                 \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a2e8f2b78bc97f27c5fa431f3af2261ed}{currentCarrotPoseIndex\_}++;
671                 ROS\_DEBUG\_STREAM(\textcolor{stringliteral}{"[BackwardLocalPlanner] setPlan: fw"} << 
      \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a2e8f2b78bc97f27c5fa431f3af2261ed}{currentCarrotPoseIndex\_});
672             \}
673 
674             ROS\_INFO\_STREAM(\textcolor{stringliteral}{"[BackwardLocalPlanner] setPlan: (found first carrot:"} << inCarrotRange << \textcolor{stringliteral}{")
      ...
      \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad9cde5c85f782cab2ddb4030e3c3f2cf}{backwardsPlanPath\_}.size());
675 
676             \textcolor{keywordflow}{return} inCarrotRange;
677         \}
\end{DoxyCode}
Here is the call graph for this function\+:
\nopagebreak
...
Definition at line 798 of file backward\+\_\+local\+\_\+planner.\+cpp.
...
\begin{DoxyCode}
799         \{
800             \textcolor{comment}{//simulate the trajectory and check for collisions, updating costs along the way}
801             \textcolor{keywordtype}{bool} end = \textcolor{keyword}{false};
802             \textcolor{keywordtype}{float} time = 0;
803             Eigen::Vector3f currentpos = pos;
804             \textcolor{keywordtype}{int} i = 0;
805             \textcolor{keywordflow}{while} (!end)
806             \{
807                 \textcolor{comment}{//add the point to the trajectory so we can draw it later if we want}
808                 \textcolor{comment}{//traj.addPoint(pos[0], pos[1], pos[2]);}
809 
810                 \textcolor{comment}{// if (continued\_acceleration\_) \{}
811                 \textcolor{comment}{//   //calculate velocities}
812                 \textcolor{comment}{//   loop\_vel = computeNewVelocities(sample\_target\_vel, loop\_vel, limits\_->getAccLimits(),
       dt);}
813                 \textcolor{comment}{//   //ROS\_WARN\_NAMED("Generator", "Flag: %d, Loop\_Vel %f, %f, %f",
       continued\_acceleration\_, loop\_vel[0], loop\_vel[1], loop\_vel[2]);}
814                 \textcolor{comment}{// \}}
815 
816                 \textcolor{keyword}{auto} loop\_vel = vel;
817                 \textcolor{comment}{//update the position of the robot using the velocities passed in}
818                 \textcolor{keyword}{auto} newpos = \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a0fe2ebd23fd79e8b574127e819ef0e14}{computeNewPositions}(currentpos, loop\_vel, dt);
819 
820                 \textcolor{keyword}{auto} dx = newpos[0] - currentpos[0];
821                 \textcolor{keyword}{auto} dy = newpos[1] - currentpos[1];
822                 \textcolor{keywordtype}{float} dist, angledist;
823 
824                 \textcolor{comment}{//ROS\_INFO("traj point %d", i);}
825                 dist = sqrt(dx * dx + dy * dy);
826                 \textcolor{keywordflow}{if} (dist > maxdist)
827                 \{
828                     end = \textcolor{keyword}{true};
829                     \textcolor{comment}{//ROS\_INFO("dist break: %f", dist);}
830                 \}
831                 \textcolor{keywordflow}{else}
832                 \{
833                     \textcolor{comment}{// ouble from, double to}
834                     angledist = angles::shortest\_angular\_distance(currentpos[2], newpos[2]);
835                     \textcolor{keywordflow}{if} (angledist > maxanglediff)
836                     \{
837                         end = \textcolor{keyword}{true};
838                         \textcolor{comment}{//ROS\_INFO("angle dist break: %f", angledist);}
839                     \}
840                     \textcolor{keywordflow}{else}
841                     \{
842                         outtraj.push\_back(newpos);
843 
844                         time += dt;
845                         \textcolor{keywordflow}{if} (time > maxtime)
846                         \{
847                             end = \textcolor{keyword}{true};
848                             \textcolor{comment}{//ROS\_INFO("time break: %f", time);}
849                         \}
850 
851                         \textcolor{comment}{//ROS\_INFO("dist: %f, angledist: %f, time: %f", dist, angledist, time);}
852                     \}
853                 \}
854 
855                 currentpos = newpos;
856                 i++;
857             \} \textcolor{comment}{// end for simulation steps}
858         \}
\end{DoxyCode}
Here is the call graph for this function\+:
\nopagebreak
...
\begin{DoxyCode}
626         \{
627             ROS\_DEBUG(\textcolor{stringliteral}{"[BackwardLocalPlanner] isGoalReached call"});
628             \textcolor{keywordflow}{return} \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad443c52ef585a8eab0364f0909222f51}{goalReached\_};
629         \}
\end{DoxyCode}
...
Definition at line 874 of file backward\+\_\+local\+\_\+planner.\+cpp.
...
\begin{DoxyCode}
875         \{
876             visualization\_msgs::Marker marker;
877             marker.header.frame\_id = this->\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a4136268882a105d9e06e173d85d5c0dc}{costmapRos\_}->getGlobalFrameID();
878             marker.header.stamp = ros::Time::now();
879 
880             marker.ns = \textcolor{stringliteral}{"my\_namespace2"};
881             marker.id = 0;
882             marker.type = visualization\_msgs::Marker::ARROW;
883             marker.action = visualization\_msgs::Marker::ADD;
884             marker.pose.orientation.w = 1;
885 
886             marker.scale.x = 0.05;
887             marker.scale.y = 0.15;
888             marker.scale.z = 0.05;
889             marker.color.a = 1.0;
890 
891             marker.color.r = 1;
892             marker.color.g = 0;
893             marker.color.b = 0;
894 
895             geometry\_msgs::Point start, end;
896             start.x = x;
897             start.y = y;
898 
899             end.x = x + 0.5 * cos(phi);
900             end.y = y + 0.5 * sin(phi);
901 
902             marker.points.push\_back(start);
903             marker.points.push\_back(end);
904 
905             visualization\_msgs::MarkerArray ma;
906             ma.markers.push\_back(marker);
907 
908             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a79f0ddfa686e2cd47bbc2f0bac9f3d4c}{goalMarkerPublisher\_}.publish(ma);
909         \}
\end{DoxyCode}
Here is the caller graph for this function\+:
\nopagebreak
...
Definition at line 588 of file backward\+\_\+local\+\_\+planner.\+cpp.
...
\begin{DoxyCode}
589         \{
590             ROS\_INFO(\textcolor{stringliteral}{"[BackwardLocalPlanner] reconfigure Request"});
591             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_abf7a5a56de2ee41afba7e63c0628ec35}{k\_alpha\_} = config.k\_alpha;
592             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a9f257183d87f1d732cb7e404f09905ad}{k\_betta\_} = config.k\_betta;
593             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad8a36184bfb011545c751109e23d3b98}{k\_rho\_} = config.k\_rho;
594             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a154043366660cc02ec758dda32817511}{enable\_obstacle\_checking\_} = config.enable\_obstacle\_checking;
595 
596             \textcolor{comment}{//alpha\_offset\_ = config.alpha\_offset;}
597             \textcolor{comment}{//betta\_offset\_ = config.betta\_offset;}
598 
599             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a0bbb80ce5bae865c4322869422803296}{carrot\_distance\_} = config.carrot\_distance;
600             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a63e30befa09c4a67cf55086923b760c7}{carrot\_angular\_distance\_} = config.carrot\_angular\_distance;
601             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_aa4ec2c87947a3c08f8278eff052e7c8c}{xy\_goal\_tolerance\_} = config.xy\_goal\_tolerance;
602             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a9c5104d328041fcde5a3c02664abad48}{yaw\_goal\_tolerance\_} = config.yaw\_goal\_tolerance;
603             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a04febb67563add6f715051a94aed1b9b}{straightBackwardsAndPureSpinningMode\_} = config.
      pure\_spinning\_straight\_line\_mode;
604 
605             \textcolor{keywordflow}{if} (\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a63e30befa09c4a67cf55086923b760c7}{carrot\_angular\_distance\_} < 
      \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a9c5104d328041fcde5a3c02664abad48}{yaw\_goal\_tolerance\_})
606             \{
607                 ROS\_WARN\_STREAM(\textcolor{stringliteral}{"[BackwardLocalPlanner] carrot\_angular\_distance ("} << 
      \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a63e30befa09c4a67cf55086923b760c7}{carrot\_angular\_distance\_} << \textcolor{stringliteral}{") cannot be lower than yaw\_goal\_tolerance ("} << 
      \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a9c5104d328041fcde5a3c02664abad48}{yaw\_goal\_tolerance\_} << \textcolor{stringliteral}{") setting carrot\_angular\_distance = "} << 
      \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a9c5104d328041fcde5a3c02664abad48}{yaw\_goal\_tolerance\_});
608                 \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a63e30befa09c4a67cf55086923b760c7}{carrot\_angular\_distance\_} = 
      \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a9c5104d328041fcde5a3c02664abad48}{yaw\_goal\_tolerance\_};
609                 config.carrot\_angular\_distance = \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a9c5104d328041fcde5a3c02664abad48}{yaw\_goal\_tolerance\_};
610             \}
611 
612             \textcolor{keywordflow}{if} (\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a0bbb80ce5bae865c4322869422803296}{carrot\_distance\_} < \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_aa4ec2c87947a3c08f8278eff052e7c8c}{xy\_goal\_tolerance\_})
613             \{
614                 ROS\_WARN\_STREAM(\textcolor{stringliteral}{"[BackwardLocalPlanner] carrot\_linear\_distance ("} << 
      \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a0bbb80ce5bae865c4322869422803296}{carrot\_distance\_} << \textcolor{stringliteral}{") cannot be lower than xy\_goal\_tolerance\_ ("} << 
      \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a9c5104d328041fcde5a3c02664abad48}{yaw\_goal\_tolerance\_} << \textcolor{stringliteral}{") setting carrot\_angular\_distance = "} << 
      \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_aa4ec2c87947a3c08f8278eff052e7c8c}{xy\_goal\_tolerance\_});
615                 \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a0bbb80ce5bae865c4322869422803296}{carrot\_distance\_} = \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_aa4ec2c87947a3c08f8278eff052e7c8c}{xy\_goal\_tolerance\_};
616                 config.carrot\_distance = \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_aa4ec2c87947a3c08f8278eff052e7c8c}{xy\_goal\_tolerance\_};
617             \}
618         \}
\end{DoxyCode}
Here is the caller graph for this function\+:
...
Definition at line 679 of file backward\+\_\+local\+\_\+planner.\+cpp.
...
\begin{DoxyCode}
680         \{
681             \textcolor{comment}{// this algorithm is really important to have a precise carrot (linear or angular)}
682             \textcolor{comment}{// and not being considered as a divergence from the path}
683 
684             ROS\_DEBUG(\textcolor{stringliteral}{"[BackwardLocalPlanner] resample precise"});
685             \textcolor{keywordflow}{if}(\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad9cde5c85f782cab2ddb4030e3c3f2cf}{backwardsPlanPath\_}.size() <=1)
686             \{
687                 ROS\_DEBUG\_STREAM(\textcolor{stringliteral}{"[BackwardLocalPlanner] resample precise skipping, size: "} << 
      \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad9cde5c85f782cab2ddb4030e3c3f2cf}{backwardsPlanPath\_}.size());
688                 \textcolor{keywordflow}{return} \textcolor{keyword}{false};
689             \}
690 
691             \textcolor{keywordtype}{int} counter = 0;
692             \textcolor{keywordtype}{double} maxallowedAngularError = 0.45 * this->
...
693             \textcolor{keywordtype}{double} maxallowedLinearError = 0.45 * this->\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a0bbb80ce5bae865c4322869422803296}{carrot\_distance\_};          \textcolor{comment}{//
       nyquist}
694 
695             \textcolor{keywordflow}{for} (\textcolor{keywordtype}{int} i = 0; i < \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad9cde5c85f782cab2ddb4030e3c3f2cf}{backwardsPlanPath\_}.size() - 1; i++)
696             \{
697                 ROS\_DEBUG\_STREAM(\textcolor{stringliteral}{"[BackwardLocalPlanner] resample precise, check: "} << i);
698                 \textcolor{keyword}{auto} &currpose = \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad9cde5c85f782cab2ddb4030e3c3f2cf}{backwardsPlanPath\_}[i];
699                 \textcolor{keyword}{auto} &nextpose = \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad9cde5c85f782cab2ddb4030e3c3f2cf}{backwardsPlanPath\_}[i + 1];
700 
701                 tf::Quaternion qCurrent, qNext;
702                 tf::quaternionMsgToTF(currpose.pose.orientation, qCurrent);
703                 tf::quaternionMsgToTF(nextpose.pose.orientation, qNext);
704 
705                 \textcolor{keywordtype}{double} dx = nextpose.pose.position.x - currpose.pose.position.x;
706                 \textcolor{keywordtype}{double} dy = nextpose.pose.position.y - currpose.pose.position.y;
707                 \textcolor{keywordtype}{double} dist = sqrt(dx * dx + dy * dy);
708 
709                 \textcolor{keywordtype}{bool} resample = \textcolor{keyword}{false};
710                 \textcolor{keywordflow}{if} (dist > maxallowedLinearError)
711                 \{
712                     ROS\_DEBUG\_STREAM(\textcolor{stringliteral}{"[BackwardLocalPlanner] resampling point, linear distance:"} << dist <<
       \textcolor{stringliteral}{"("} << maxallowedLinearError << \textcolor{stringliteral}{")"} << i);
713                     resample = \textcolor{keyword}{true};
714                 \}
715                 \textcolor{keywordflow}{else}
716                 \{
717                     \textcolor{keywordtype}{double} currentAngle = tf::getYaw(qCurrent);
718                     \textcolor{keywordtype}{double} nextAngle = tf::getYaw(qNext);
719 
720                     \textcolor{keywordtype}{double} angularError = fabs(angles::shortest\_angular\_distance(currentAngle, nextAngle));
721                     \textcolor{keywordflow}{if} (angularError > maxallowedAngularError)
722                     \{
723                         resample = \textcolor{keyword}{true};
724                         ROS\_DEBUG\_STREAM(\textcolor{stringliteral}{"[BackwardLocalPlanner] resampling point, angular distance:"} << 
      angularError << \textcolor{stringliteral}{"("} << maxallowedAngularError << \textcolor{stringliteral}{")"} << i);
725                     \}
726                 \}
727 
728                 \textcolor{keywordflow}{if} (resample)
729                 \{
730                     geometry\_msgs::PoseStamped pintermediate;
731                     \textcolor{keyword}{auto} duration = nextpose.header.stamp - currpose.header.stamp;
732                     pintermediate.header.frame\_id = currpose.header.frame\_id;
733                     pintermediate.header.stamp = currpose.header.stamp + duration *0.5;
734 
735                     pintermediate.pose.position.x = 0.5 * (currpose.pose.position.x + nextpose.pose.
      position.x);
736                     pintermediate.pose.position.y = 0.5 * (currpose.pose.position.y + nextpose.pose.
      position.y);
737                     pintermediate.pose.position.z = 0.5 * (currpose.pose.position.z + nextpose.pose.
      position.z);
738                     tf::Quaternion intermediateQuat = tf::slerp(qCurrent, qNext, 0.5);
739                     tf::quaternionTFToMsg(intermediateQuat, pintermediate.pose.orientation);
740 
741                     this->\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad9cde5c85f782cab2ddb4030e3c3f2cf}{backwardsPlanPath\_}.insert(this->
...
742 
743                     \textcolor{comment}{// retry this point}
744                     i--;
745                     counter++;
746                 \}
747             \}
748 
749             ROS\_DEBUG\_STREAM(\textcolor{stringliteral}{"[BackwardLocalPlanner] End resampling. resampled:"} << counter << \textcolor{stringliteral}{" new
...
Definition at line 758 of file backward\+\_\+local\+\_\+planner.\+cpp.
...
\begin{DoxyCode}
759         \{
760             ROS\_INFO\_STREAM(\textcolor{stringliteral}{"[BackwardLocalPlanner] setPlan: new global plan received ("} << plan.size() << \textcolor{stringliteral}{
      ")"});
761             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_aa0e2b0589fce0bb228eb87751528e822}{initialPureSpinningStage\_} = \textcolor{keyword}{true};
762             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad443c52ef585a8eab0364f0909222f51}{goalReached\_} = \textcolor{keyword}{false};
763             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad9cde5c85f782cab2ddb4030e3c3f2cf}{backwardsPlanPath\_} = plan;
764             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ae4d7254afd19ae1f7bd8eac9f5d58b9a}{inGoalPureSpinningState\_} =\textcolor{keyword}{false};
765 
766             \textcolor{comment}{// find again the new carrot goal, from the destiny direction}
767             tf::Stamped<tf::Pose> tfpose = \hyperlink{namespacecl__move__base__z_1_1backward__local__planner_afd5bb2a0d144f7b89f4215fbcb1fbe69}{optionalRobotPose}(
      \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a4136268882a105d9e06e173d85d5c0dc}{costmapRos\_});
768 
769             geometry\_msgs::PoseStamped posestamped;
770             tf::poseStampedTFToMsg(tfpose,posestamped);
771             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad9cde5c85f782cab2ddb4030e3c3f2cf}{backwardsPlanPath\_}.insert(\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ad9cde5c85f782cab2ddb4030e3c3f2cf}{backwardsPlanPath\_}.begin(), 
      posestamped);
772 
773             this->\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a100fe8bdaaf89b0fa54ecf2aef0966ed}{resamplePrecisePlan}();
774 
775             \hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a2e8f2b78bc97f27c5fa431f3af2261ed}{currentCarrotPoseIndex\_} = 0;
776             this->\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a35832ebefc43e5977e1222389244c8d3}{resetDivergenceDetection}();
777 
778             \textcolor{keywordflow}{if} (plan.size() == 0)
779             \{
780                 ROS\_WARN(\textcolor{stringliteral}{"[BackwardLocalPlanner] received plan without any pose"});
781                 \textcolor{keywordflow}{return} \textcolor{keyword}{true};
782             \}
783 
784             \textcolor{keywordtype}{bool} foundInitialCarrotGoal = this->\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_a0025cd3ffaa04a6a02492a33673a78a1}{findInitialCarrotGoal}(tfpose);
785             \textcolor{keywordflow}{if} (!foundInitialCarrotGoal)
786             \{
787                 ROS\_ERROR(\textcolor{stringliteral}{"[BackwardLocalPlanner] new plan rejected. The initial point in the global path
       is too much far away from the current state (according to carrot\_distance parameter)"});
788                 \textcolor{keywordflow}{return} \textcolor{keyword}{false}; \textcolor{comment}{// in this case, the new plan broke the current execution}
789             \}
790             \textcolor{keywordflow}{else}
791             \{
792                 this->\hyperlink{classcl__move__base__z_1_1backward__local__planner_1_1BackwardLocalPlanner_ab1b01ecdb27641dde60661362d75465b}{divergenceDetectionUpdate}(tfpose);
793                 \textcolor{comment}{// SANDARD AND PREFERED CASE ON NEW PLAN}
