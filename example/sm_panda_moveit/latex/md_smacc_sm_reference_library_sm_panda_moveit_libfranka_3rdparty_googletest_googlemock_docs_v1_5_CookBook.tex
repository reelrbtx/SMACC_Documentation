You can find recipes for using Google Mock here. If you haven\textquotesingle{}t yet, please read the For\+Dummies document first to make sure you understand the basics.
...
{\bfseries I\+M\+P\+O\+R\+T\+A\+NT N\+O\+TE\+:} What we describe in this recipe is {\bfseries N\+OT} true yet, as Google Mock is not currently thread-\/safe. However, all we need to make it thread-\/safe is to implement some synchronization operations in {\ttfamily $<$\hyperlink{gtest-port_8h}{gtest/internal/gtest-\/port.\+h}$>$} -\/ and then the information below will become true.
...
\hline
\rowcolor{\tableheadbgcolor}\textbf{ {\ttfamily \%\%} }&\textbf{ a single {\ttfamily \%} character  }\\\cline{1-2}
\endfirsthead
\hline
\endfoot
\hline
...
For example, 
\begin{DoxyCode}
MATCHER\_P2(InClosedRange, low, hi, "is in range [%(low)s, %(hi)s]") \{
